...
router.get('/gallery', (req, res) => {
  try {
    res.json(getDb().prepare('SELECT * FROM gallery WHERE is_active=1 ORDER BY sort_order ASC').all());
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/book', (req, res) => {
  ...
  const db = getDb();
  const trip = db.prepare('SELECT t.*, a.agency_name, a.commission_rate FROM trips t JOIN agencies a ON t.agency_id=a.id WHERE t.id=? AND t.available_seats>=? AND a.is_active=1').get(trip_id, seats);
...
// POST /api/public/pay — Commission calculée ici, dès le paiement
router.post('/pay', async (req, res) => {
  ...
  if (payment_method === 'cash') {
    ...
    try {
      ...
      if (orig && orig.data && ['202','200'].includes(String(orig.data.statusCode))) {
        ...
        const msg = (orig && orig.data && orig.data.statusDescription) || 'Paiement refusé';
  ...
  try {
    ...
    console.log(`💳 Paiement ${booking.reference} | Total: ${booking.total_price} FC | Commission Nzela (${rate}%): ${commission_amount} FC | Net agence: ${net_agency} FC`);
