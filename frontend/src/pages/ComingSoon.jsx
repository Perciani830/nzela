...
function getRapides(currency) {
  ...
  return [500, 1000, 2000, 5000]; // CDF · XAF · XOF
}
...
function ContribModal({ onClose }) {
  ...
  // Réinitialise les champs liés au pays quand on change de pays
    ...
    if (payMethod === 'mobile') {
      try {
        ...
        try {
          ...
          // Erreur réseau pure (pas de connexion, CORS preflight bloqué, Railway down…)
        ...
        try {
          ...
          console.error('[Nzela] Réponse non-JSON, status:', res.status);
      ...
      <div style={{ background:'linear-gradient(160deg,#0d1f16,#081220)', border:'1px solid rgba(61,170,106,0.3)', borderRadius:24, padding:36, width:'100%', maxWidth:460, position:'relative', maxHeight:'92vh', overflowY:'auto', boxShadow:'0 32px 80px rgba(0,0,0,0.7)' }}>
        ...
        {step === 1 && (
          <>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:6 }}>
                  {PAYS.map(p => (
                    <button key={p.code}
                      ...
                      style={{ padding:'10px', borderRadius:12, fontWeight:600, fontSize:14, cursor:'pointer', transition:'all 0.2s',
            ...
            {/* Devise (affichée uniquement pour carte, sinon dérivée du pays) */}
            {payMethod === 'card' && (
              <div style={{ marginBottom:18 }}>
                <label style={LBL_STYLE}>Devise</label>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {/* Pour carte, on garde CDF / USD comme options simples */}
                  {[
                    { code:'CDF', flag:'🇨🇩', label:'Franc CDF' },
                    { code:'USD', flag:'🇺🇸', label:'Dollar USD' },
                  ].map(c => (
                    <button key={c.code}
                      ...
                      style={{ padding:'11px', borderRadius:12, fontWeight:700, fontSize:15, cursor:'pointer', transition:'all 0.2s',
                        background: currency === c.code ? '#3DAA6A' : 'rgba(61,170,106,0.07)',
                        color:      currency === c.code ? '#050E17' : '#3DAA6A',
                        border:    `1px solid ${currency === c.code ? '#3DAA6A' : 'rgba(61,170,106,0.2)'}`,
                      }}>
                      {c.flag} {c.label}
                    </button>
                  ))}
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    {ops.map(op => (
                      <button key={op.id} onClick={() => set('operator', op.id)}
                        ...
                        <img src={op.logo} alt={op.label} style={{ width:22, height:22, objectFit:'contain' }}
            ...
            <button onClick={submit}
              style={{ width:'100%', padding:'14px', borderRadius:14, border:'none', background:'#3DAA6A', color:'#050E17', fontWeight:800, fontSize:16, cursor:'pointer', fontFamily:'Plus Jakarta Sans,sans-serif', transition:'opacity 0.2s' }}
      ...
      <section style={{ position:'relative', zIndex:1, padding:'72px 0 80px' }}>
        ...
        <div style={{ textAlign:'center' }}>
          <button className="cs-ghost-btn" onClick={() => setShowContrib(true)}
            style={{ background:'rgba(61,170,106,0.07)', color:'#3DAA6A', border:'1px solid rgba(61,170,106,0.25)', borderRadius:100, padding:'13px 34px', fontWeight:700, fontSize:15, cursor:'pointer', fontFamily:'Plus Jakarta Sans,sans-serif' }}>
