    ...
    <div style={{minHeight:'100vh',background:'var(--night)'}}>
      ...
      <nav style={{position:'sticky',top:0,zIndex:50,height:52,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 22px',background:'rgba(5,14,23,.92)',backdropFilter:'blur(14px)',borderBottom:'1px solid var(--border)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <a href="/login" className="btn btn-ghost" style={{fontSize:12,padding:'6px 13px'}}>Agences →</a>
        </div>
      </nav>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:9}}>
                <div className="input-group"><label className="lbl">📍 Départ</label>
                  <select className="field" value={search.from} onChange={e=>setSearch({...search,from:e.target.value})}>
                    <option value="">Ville de départ</option>{CITIES.map(c=><option key={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  <div style={{flex:1,height:1,background:'var(--border)'}}/>
                  <button onClick={swap} style={{background:'var(--green-bg)',border:'1px solid rgba(61,170,106,.18)',borderRadius:99,padding:'3px 11px',cursor:'pointer',fontSize:12,color:'var(--green-l)',transition:'var(--ease)',fontFamily:'var(--font)',fontWeight:600}}
                    onMouseEnter={e=>e.currentTarget.style.background='rgba(61,170,106,.18)'}
                    onMouseLeave={e=>e.currentTarget.style.background='var(--green-bg)'}>⇅ Inverser</button>
                  <div style={{flex:1,height:1,background:'var(--border)'}}/>
                </div>
                <div className="input-group"><label className="lbl">📍 Arrivée</label>
                  <select className="field" value={search.to} onChange={e=>setSearch({...search,to:e.target.value})}>
                    <option value="">Ville d'arrivée</option>{CITIES.map(c=><option key={c}>{c}</option>)}
                  </select>
                </div>
                <div className="input-group"><label className="lbl">📅 Date</label>
                ...
                <button className="btn btn-primary w100" onClick={handleSearch} disabled={loading} style={{justifyContent:'center',height:40,fontSize:13,borderRadius:'var(--r)',marginTop:2}}>
